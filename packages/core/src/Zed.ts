...
export type ZGettable<GetSchema extends JSONSchema, GetOptions> = {
  ...
  valueSchema: GetSchema;
  get: (options: GetOptions) => Promise<FromSchema<GetSchema>>;
  getChild: <K extends keyof FromSchema<GetSchema>>(
    key: K,
    options: GetOptions
  ) => Promise<FromSchema<GetSchema>[K] | undefined>;
};
...
export type ZCallRecord = {};

export type ZCallsCollection = ZGettableGroup<ZCallRecord>;
...
export function createZGettable<StateSchema extends JSONSchema7, GetOptions>(
  ...
  return {
    ...
    get,
    valueSchema,
    getChild: async <K extends keyof FromSchema<StateSchema>>(
      key: K,
      options: GetOptions
    ): Promise<FromSchema<StateSchema>[K]> => {
      // @ts-ignore
      const value: FromSchema<StateSchema> = await get(options);
      return value[key];
    },
  };
}
...
export function createZContainer<Zeds extends Record<string, AnyZed>>(
  ...
  return {
    ...
    z,
    meta: undefined,
    get: async <S extends keyof Zeds>(zedKey: S) => {
      if (z[zedKey] === undefined)
        throw new Error(`Cannot find ${zedKey.toString()} in Zeds`);
      return z[zedKey];
    },
  };
}
