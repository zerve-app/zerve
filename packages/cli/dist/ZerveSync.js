...
var packagePath = (0, import_path.join)(projectPath, "package.json");
...
function readJSONFile(jsonPath) {
  try {
    const packageData = (0, import_fs_extra.readFileSync)(packagePath, "utf8");
    const modulePackage2 = JSON.parse(packageData);
    return modulePackage2;
  } catch (e) {
    fail(`Could not read JSON from ${jsonPath}`);
  }
}
var modulePackage = readJSONFile(packagePath);
if (!modulePackage.zerve)
  fail(`package.json does not have a zerve configuration`);
var { zerve: localZerveConfig } = modulePackage;
var zerveGeneratedDir = (0, import_path.join)(projectPath, "zerve");
console.log("==== Zerve-Sync ====");
if (!(0, import_fs_extra.existsSync)(zerveGeneratedDir)) {
  console.log('> Creating "zerve" directory');
  (0, import_fs_extra.mkdirSync)(zerveGeneratedDir);
}
Promise.all(Object.entries(localZerveConfig.dynamicSync).map(async ([zDynamicName, url]) => {
  console.log(`== Syncronizing ./zerve/${zDynamicName} from ${url} ==`);
  const parsedURL = String(url).match(/^(https?:\/\/)([^\/]+)\/(.+)$/);
  if (!parsedURL)
    fail(`Invalid dynamicSync URL: ${url}`);
  const [_fullUrl, protocol, origin, path] = parsedURL;
  const zURL = `${protocol}${origin}/.z/${path}`;
  const res = await (0, import_node_fetch.default)(zURL);
  const zClientPath = (0, import_path.join)(zerveGeneratedDir, zDynamicName);
  if (!(0, import_fs_extra.existsSync)(zClientPath)) {
    console.log(`> Creating "zerve/${zDynamicName}" client directory`);
    (0, import_fs_extra.mkdirSync)(zClientPath);
  }
  const zStoreFullData = await res.json();
  const { $schemas: zStoreSchemas, ...zStoreData } = zStoreFullData;
  const zFileValues = Object.fromEntries(Object.entries(zStoreData).map(([fileName, file]) => [
    fileName,
    file.value
  ]));
  const zFileSchemas = Object.fromEntries(Object.entries(zStoreData).map(([fileName, file]) => [
    fileName,
    file.schema
  ]));
...
${Object.entries(zFileSchemas).map(([fileName, fileSchema]) => `export const ${fileName} = zClient.createAccessor("${fileName}", ${JSON.stringify(fileSchema, null, 2)});`).join("\n")}

export const FF = zClient.createAccessor('FF');
`;
  console.log(`> Writing "zerve/${zDynamicName}/index.ts"`);
  (0, import_fs_extra.writeFileSync)((0, import_path.join)(zClientPath, "index.ts"), zClientFileData);
  console.log(`> Writing "zerve/${zDynamicName}/data-sync.json"`);
  (0, import_fs_extra.writeFileSync)((0, import_path.join)(zClientPath, "data-sync.json"), JSON.stringify(zFileValues, null, 2));
  console.log(`> Writing "zerve/${zDynamicName}/schema-sync.json"`);
  (0, import_fs_extra.writeFileSync)((0, import_path.join)(zClientPath, "schema-sync.json"), JSON.stringify({
