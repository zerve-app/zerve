...
if (!process.argv[2]) {
  console.log(`
 JSON.stringify and base64 build args:
 
 
type BuildArgs = {
  apiEnv: Record<string, string | number>;
  webEnv: Record<string, string | number>;
  buildId: string;
  time: number;
  gitId: string;
  buildPath: string;
...
`);
  throw new Error("Invalid Setup Aruments.");
}

const buildArgs = JSON.parse(
...
buildAndDeployService(buildArgs).catch((e) => {
  console.error(e);
  process.exit(1);
});

// const apiService =  createServiceFile('API', {}, '/home/zerve/')
