...
export async function getStaticPaths() {
  const frontmatters = await getAllFrontmatter('docs/boosted')
  console.log('FMS', JSON.stringify(frontmatters, null, 2))
  const paths = frontmatters.map((matter) => ({
    params: { slug: matter.slug.join('/').replace('docs/boosted/', '').split('/') },
  }))
...
export async function getStaticProps(context) {
  const { frontmatter, code } = await getMdxBySlug('docs/boosted', context.params.slug.join('/'))
  return {
    props: {
      frontmatter,
      code,
